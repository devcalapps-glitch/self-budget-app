...
- **100% Zero-Cost Cloud Sync & File Backup**: Schema-versioned, cent-safe JSON serialization engine (`SyncDataPayload.kt`) backed by `CloudSyncManager.kt`. Users can export/import full Room DB backups to Google Drive `appDataFolder` or Android local storage with $0.00 infrastructure cost.
...
### 3.7 Settings, Appearance & Data Backup / Cloud Sync
- **100% Zero-Cost Cloud Sync & Data Backups**: Export full database snapshot to `.json` backup file via Android Share Sheet or Google Drive `appDataFolder`; 1-tap restore using System Document Picker.
...
| **Cloud Sync & Serialization** | `CloudSyncManager` + Gson cent-safe JSON serialization engine (`SyncDataPayload`) |
...
- **Zero-Cost Google Drive Cloud Sync**: Uses private Google Drive `appDataFolder` / Android Storage Access Framework for zero server costs.
