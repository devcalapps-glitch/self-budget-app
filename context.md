...
- **Unified Universal Month Navigation**: Consistent top `MonthYearHeader` selector (`< August 2026 >`) across all main tabs (Dashboard, Plan/Budget, Recurring, Analytics), enabling seamless month-by-month navigation anywhere in the app.
- **Cycle-Aware Recurring Expenses & Date Selector**: Due recurring bills/paychecks feature start/due date pickers, cycle-aware status badges (`Posted for Aug ✅`), disabled posting actions for archived bills, quick preset chips (`+$50, +$100, +$250, +$500, +$1000`), and clean muted card styling for archived items.
...
- **Clean Accounts & Wallets View**: Dedicated `AccountsViewAllModal` displaying full account cards with search without cluttered account type filter pills.
